...
# ------------------------------------------------
# PATH CONFIG (MATCHES YOUR DOCKER MOUNTS)
# ------------------------------------------------
ROOT = "/app"

# These are the directories you mapped in docker run:
#   -v /mnt/user/appdata/seapay/data:/data
#   -v /mnt/user/appdata/seapay/output:/output
#   -v /mnt/user/appdata/seapay/pdf_template:/templates
#   -v /mnt/user/appdata/seapay/config:/config
DATA_DIR = "/data"
OUTPUT_DIR = "/output"
...
def match_ship(text: str) -> str | None:
    text = normalize(text)
    words = text.split()
...
def year_from_filename(fn: str) -> str:
...
def get_rate(name: str, rates: dict[str, str]) -> str:
...
# PDF GENERATION
# ------------------------------------------------
def make_pdf(group, name: str, rate: str):
    ...
    filename = f"{prefix}{last}_{first}_{ship}_{start}_TO_{end}.pdf".replace(
        " ", "_"
    )
    ...
    c.drawString(
        38.8,
        595,
        f"____. REPORT CAREER SEA PAY FROM {start} TO {end}.",
    )
    c.drawString(
        64,
        571,
...
# Map folder names used in URLs to actual directories
FOLDERS = {
    "data": DATA_DIR,
    "output": OUTPUT_DIR,
}
...
def upload():
    """Upload input PDFs into DATA_DIR."""
